...
namespace OpenMesh
{
	...
	SmartFaceHandle
		DebuggableTriConnectivity::add_face(const VertexHandle* _vertex_handles, size_t _vhs_size)
	{
		...
		/// face is triangle -> ok
		if (_vhs_size == 3)
			return DebuggablePolyConnectivity::add_face(_vertex_handles, _vhs_size);
		...
		else
		{
			...
			while (i < _vhs_size)
			{
				...
				fh = DebuggablePolyConnectivity::add_face(vhandles, 3);
			}
	...
	SmartFaceHandle DebuggableTriConnectivity::add_face(const std::vector<VertexHandle>& _vhandles)
	{
	...
	SmartFaceHandle DebuggableTriConnectivity::add_face(const std::vector<SmartVertexHandle>& _vhandles)
	{
	...
	SmartFaceHandle DebuggableTriConnectivity::add_face(VertexHandle _vh0, VertexHandle _vh1, VertexHandle _vh2)
	{
		VertexHandle vhs[3] = { _vh0, _vh1, _vh2 };
		return DebuggablePolyConnectivity::add_face(vhs, 3);
	}

	//-----------------------------------------------------------------------------

	bool DebuggableTriConnectivity::is_collapse_ok(HalfedgeHandle v0v1)
	{
	...
	//-----------------------------------------------------------------------------
	DebuggableTriConnectivity::HalfedgeHandle
		DebuggableTriConnectivity::vertex_split(VertexHandle v0, VertexHandle v1,
	...
	//-----------------------------------------------------------------------------
	DebuggableTriConnectivity::HalfedgeHandle
		DebuggableTriConnectivity::insert_loop(HalfedgeHandle _hh)
	{
	...
	//-----------------------------------------------------------------------------
	DebuggableTriConnectivity::HalfedgeHandle
		DebuggableTriConnectivity::insert_edge(VertexHandle _vh, HalfedgeHandle _h0, HalfedgeHandle _h1)
	{
	...
	//-----------------------------------------------------------------------------
	bool DebuggableTriConnectivity::is_flip_ok(EdgeHandle _eh) const
	{
	...
	//-----------------------------------------------------------------------------
	void DebuggableTriConnectivity::flip(EdgeHandle _eh)
	{
	...
	void DebuggableTriConnectivity::split(EdgeHandle _eh, VertexHandle _vh)
	{
	...
	void DebuggableTriConnectivity::split_copy(EdgeHandle _eh, VertexHandle _vh)
	{
